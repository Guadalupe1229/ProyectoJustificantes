...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO solicitudes2 (nombre, grupo, motivo, fecha_ausencia) VALUES (?, ?, ?, ?)',
    ...
    const [rows] = await pool.query(
      'SELECT * FROM solicitudes2 WHERE id = ?',
      [result.insertId]
    );
...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT * FROM solicitudes2 ORDER BY fecha_solicitud DESC'
    );
    res.json(rows);
  } catch (err) {
